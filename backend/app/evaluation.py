...
class EvaluationHarness:
    ...
    async def _evaluate_case(self, case: EvaluationCase) -> EvaluationResult:
        ...
        # Avalia citações
        citation = self._evaluate_citations(
            response.sources,
    ...
    def _calculate_summary(
        ...
        for cat, cat_results in by_category.items():
            results_by_category[cat] = {
                ...
                "avg_correctness": sum(r.correctness_score for r in cat_results) / len(cat_results),
                "avg_citation": sum(r.citation_score for r in cat_results) / len(cat_results),
                "avg_response_time_ms": sum(r.response_time_ms for r in cat_results) / len(cat_results)
            }
